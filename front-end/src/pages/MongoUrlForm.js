...
//** Summit form handle function
  const formSubmitHandler = (e) => {
    e.preventDefault();
    const url = `http://localhost:8080/mongodb-user-url`;
    const userMongoData = { AccountURL, AccountName };
    setIspending(true);
    ...
    fetch(url, {
      ...
      .then((data) => {
        ...
        if(data.ConnectionStatus === true){
            navigate(`/mongodb-url-login-details`)
        }
      })
      .catch((err) => {
        setIspending(false);
      ...
      <Grid container>
        ...
        <Grid item xs={12} sm={12} md={12} lg={6} className={classes.formSection}>
          ...
          <form onSubmit={formSubmitHandler} method="POST" className={classes.form}>
            ...
            <TextField label="Connection String URL" variant="filled" fullWidth color="secondary" margin="dense" multiline name="url-name" onChange={(e) => setAccountURL(e.target.value)} required/>
          ...
          <div className="pd-1">
            <Typography variant="body1" className={classes.smalltext}>
              {`eg url : "mongodb+srv://<USERNAME>:<PASSWORD>@<CLUSTERNAME>.wvunv.mongodb.net/<DATABASE>?retryWrites=true&w=majority"`}
            </Typography>
